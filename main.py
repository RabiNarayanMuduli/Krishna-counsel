    ...
    def _generate_and_play(self, text):
        try:
            ...
            try:
                ...
                time.sleep(0.2)  # small delay for Windows to release file

                if os.path.exists(filename):
                    os.remove(filename)

            except Exception as e:
                print("File cleanup warning:", e)

            if self.app_running and hasattr(self, "avatar"):
                if self.avatar.label.winfo_exists():
                    self.avatar.change_avatar(self.idle_gif)
